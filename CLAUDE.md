...
* **A day a moderator names is kept as a date.** "This Sunday" is true for a week; written down as they said it, the answer is wrong the following month and there is nothing in it to be wrong about — asked which date, the bot can only repeat the phrase. So before a moderator's answer becomes knowledge, its relative days are read against today in the guild's timezone and put back to them in one line ("this sunday is Sunday 13 September? Tick it and that is what I will remember"); the tick keeps the resolved form. Nothing is guessed: a phrase that names a weekday must land on that weekday, a date must be within about a year, and the phrase must be in the sentence, or the words are kept exactly as they were written.
...
6. **The beacon, everywhere it belongs.** The object from moment 2 is the product's one device, and it is the same component, the same geometry, the same materials and the same post pass wherever it appears; nothing new is drawn for it. Its light has four states: **off**, unconfigured; **amber**, the vard watching; **working**, amber with a slow pulse (1.4s, shallow) for the seconds while something is actually being carried out; and **green**, the result of an answer. Green is never a decoration. The slit lights in fifths from the bottom, so setup can show what has been decided rather than describe it.
   ...
   **Setup is its own place**, at `/setup/[guildId]`: no sidebar, no dashboard chrome, one decision on screen at a time, and the hero's scene held for the whole flow — the cliff, the coded sky, the beacon standing right of centre on the ground with its contact shadow, large (about 55% of the viewport's height), and never moving. There is no paper and no flat page anywhere in it. The slit is the progress: dark at the door, one fifth in amber per thing decided (name, voice, language, knowledge, scope) from the bottom up, growing over 400ms; full amber at the test; a crossfade to green when the bot arrives. The lit part is solid amber and the rest is dark, with nothing outlining the whole slit. No other progress bar and no "n of 5" line: the object says it. The door is one line, "Kalvard isn't set up yet.", and one button, centred in the sky; the form is offered once, quietly, inside the first question. Every step lives on one glass panel — the same element for the whole flow, never unmounted between questions — on the left half of the scene: night at 60%, blur 6px, one 1px specular top edge, radius 16, the night shadow, max 620 wide, vertically centred, 32px padding, as tall as its content between 220px and 70vh, its height moving over 300ms and a new step's contents fading in over 200ms once it settles. Kalvard's current question is 26px star white with the green rule and is the dominant thing on screen; the exchanges above it are 16px in ink soft. The input is 52px, inside the panel, 20px under the conversation, transparent with a hairline that turns amber on focus and a star-white Send inside it. What has been decided stands on a second glass directly under the beacon, centred on it, 320 wide, listing only the fields that exist. Setup's buttons are 52px tall, 24px sides, 16px text, radius 12, star white with ink text and a 1px specular top edge, hover brightens 6%, active scales 0.98, centred under the text on the door and the success screen. Every step past the first can be skipped.

   **The last of setup is three questions, not a form**: where members get help (a ticket system, a help channel, or one they already have), then whichever channel that answer needs — one of theirs or one Kalvard makes, with its category — and then who it wakes when it is not sure and where it reports quietly. The list of channels it may answer in is a setting, not a question anybody wants at the end of setup: it lives on the Settings page.

   **The order is: decide, bring it, where members get help, try it, put it live.** The test chat is the last thing before the bot goes live, so what you are trying is the thing that is about to answer members. Going live is violet: the slit flickers fast in violet for the seconds it takes to arrive, then crosses to green when it is alive. While Kalvard is thinking, the slit flickers in amber rather than the panel simply pausing. `/dev/setup` walks the whole flow with a scripted conversation, a simulated read of a file, and the support questions worked out in the browser: it is the whole thing end to end and it touches nothing.

   **Nothing is shown until everything is.** On first load the screen is the night scene with the slit breathing faintly (amber at 15%, a 1.6s cycle) and nothing else. The page waits for the photograph to be decoded, the fonts to load, the WebGL scene to compile and paint its first frame, and whatever that screen had to fetch; then the whole screen appears at once in a single 500ms fade — panel, card and text together — and the breath resolves into the real light. Everything below the gate is laid out before the fade, so revealing it moves nothing, and after it nothing else appears. The hero page is gated the same way.

   **The knowledge step takes documents.** Three ways in, inside the panel: a paperclip that opens the file picker (txt, md, pdf, several at once), a paste — anything over 200 characters typed into the input is knowledge and not an answer to the question on screen — and a file dropped anywhere on the panel, with the panel taking an amber edge while it is over it. A file dropped at any other step is kept and read when the knowledge step comes, never lost. Each one is stored, read and chunked at once, with its name and a thin amber bar in the panel while it is read and "Ready" after; a failure says what went wrong and offers another go. Kalvard then confirms in one line how many pieces it read ("Read your rules, 18 pieces"), runs the gap analysis on that document alone, once, and asks about the biggest gap. The knowledge fifth lights as soon as the first document is ready. "Not yet, I'll add it later" stays available and lights nothing.

   **The conversation never waits on the network to look alive.** What the owner types appears in the panel the moment they press Enter and the input clears; while Kalvard thinks, the panel holds the space the reply will take and a cursor blinks in it; the reply types itself out at the hero's rhythm rather than landing as a block. While a request is in flight the scene drops to 30fps, so the sky's drift never costs the reply a frame.
...
12d. **The series.** One shipped workflow, **Bo3 series**, that runs a whole best-of-three between two teams from greeting to results, and is the first workflow that touches things outside Discord. A team is a Discord role; the match is two roles and a channel, and the members holding a team's role are the team. The flow, in order, each step a `do`, `ask`, `wait_for`, `if` or `pick` from 12c:

   1. Greet both teams by role mention, read back the Bo3 rules from the knowledge, and coin-flip blue and red side for game 1, announced.
   2. Open a draft session on the guild's draft site and post the blue link to the blue team and the red link to the red team. After one minute with the draft not started, ask once whether everything is fine; after another minute, mention both teams and ask again. A yes gets "good luck" and nothing else.
   3. When the draft is done, post its result in the channel (the site's own picture of it when it has one, otherwise a card Kalvard draws from the picks), and tell the blue team to create the lobby and give the other team the code. Two minutes later ask once whether the game is starting; no answer is fine. Anything said after that gets one "is the game over?"; a yes without a screenshot gets one ask for the screenshot.
   4. Wait for a screenshot. The picture is read by the model, not by an OCR library: is it a Wild Rift end-of-game screen, and does it say victory or defeat? Anything else is refused with one line and the wait continues. The poster's team is the perspective: "defeat" from a member of team A is a win for team B. The screenshot is stored, the game recorded.
   5. Ask the losing team, as buttons, which side they want for the next game, announce it, and go round again from step 2 with the links matched to the new sides.
   6. When one team reaches two wins: "gg", the score, and a post in the results channel as `WINNER 2-0 LOSER` with the stored screenshots below it, and nothing else. Only screenshots the model recognised as end screens are ever posted, and one member reports for the whole series: the first person to post a valid screenshot is the reporter, and a screenshot from anyone else is refused, so a duplicate never counts twice.

   **The draft site is a data source**, kind `draft_flow`, with the guild's base URL and a key in its config: create a session, read its state (waiting, drafting, done, with the picks and an image when there is one), all through `fetch_data` under the same rules as every other source (https, allowlisted host, four-second deadline, size cap). Pointed at `fixture:draft-flow` it reads `evals/fixtures/draft-flow.json`, which is what the eval runs against, and it is how the flow is built before the site has an API. What the site must expose is written in `packages/core/src/fetchers/draft-flow.ts` as the contract.

   **The keeper.** A channel where a run is alive belongs to that run, whichever workflow it is: every message there ends with the keeper, and the support loop — its role menus, its clarifying questions, its command plans — never speaks in it. The room is the first channel the run spoke in, kept with the run whatever it waits on. Every shipped workflow carries a brief and rules; one without gets a plain brief naming the routine, and its memory is its plain variables. While a run is alive in a channel, Kalvard is the admin at the table: every message there is read, without a mention, and most are left alone. A workflow carries a `brief` (what Kalvard is in that channel) and `rules` (what the run enforces), both in the owner's words; the run's own memory (sides, draft state, score, who reports, what it is waiting for) is what it knows. It answers a question about the series from that memory, a question about the rules from the rules, and a question about the server from the knowledge, in one or two lines, like a person; what none of those hold, or only a person decides — a remake, a forfeit, an exception — goes to the moderators. Players' words never move the run; a moderator's do, in two ways only: time granted on the current wait, and a fact remembered for the rest of the run. In code, not prompt: a player is never granted anything, one unprompted line a minute at most, never the same line twice, and a question put to it directly is always answered. `pnpm --filter @kalvard/core eval:keeper` runs a scripted match channel: banter is ignored, the draft state and the sides come from memory, a player asking for time gets the rule and not the minutes, a moderator granting fifteen minutes is an act of fifteen minutes, a remake goes to staff, and the code guards hold with the model out of the way.

   **From the calendar.** The guild's match calendar is a `rift_legends` source (the contract in `fetchers/rift-legends.ts`, now with `discordRoleId` on a team and `POST /matches/:id/result`), configured once in the environment (`CALENDAR_URL`, `CALENDAR_KEY`) or as a source of its own. The bot's clock reads it every minute: five hours before a match it makes `#teama-vs-teamb` in `guild_settings.match_category` (default "Matches", made if missing), private to the two team roles, the moderators and Kalvard — a team is the role the calendar names, or the role carrying the team's name, and a team with no role is reported to the moderators rather than guessed — posts the greeting and says when the draft opens, and the series waits on the clock (`wait_clock`) for the hour. Every match is claimed by id, so a restart never makes a second room. When the series ends the result and the recognised screenshots are reported to the calendar. The Workflows page lists the calendar's coming matches with where each has got to. Units cover the arithmetic (due at five hours, not six, never once played; the channel's name; a team's role); `eval:workflow` covers a series that greets at once and flips the coin at the hour.

   **The eval that must pass**, `pnpm --filter @kalvard/core eval:workflow`, as a scripted flow with the fixture and two fake teams: the greeting names both roles; the coin flip is announced; both links go to the right teams; the one-minute and two-minute nudges fire on the scripted silences and not on the scripted answers; a screenshot that the model calls "not an end screen" is refused and the wait holds; a "defeat" posted by a member of team A records a win for team B; the losing team is asked for its side as buttons; a second valid screenshot from a different member is refused as not the reporter; and after two wins the results channel gets exactly `WINNER 2-0 LOSER` and the two stored screenshots. With `post_message` switched off it stops and names it; with the results channel gone it stops and names it.
12f. **Where members get help.** One exclusive choice per server, in onboarding (the finish screen) and on Settings under "Where members get help": **A ticket system** (Kalvard creates the category, the channel with the "Open a ticket" button, and the permissions), **A help channel** (Kalvard creates `#help` and answers publicly there), or **A channel I already have** (pick from existing channels; nothing is created). Stored as `guild_settings.support_mode` (`tickets` | `help_channel` | `existing_channel`) plus `support_channel_id`, and `support_setup` for what the choice made. Kalvard never picks the structure itself: it asks one question at a time, each with a sensible default the owner can accept or change — for tickets, which category (existing ones listed, or "create a new one called X"), which channel holds the button (existing, or create one), whether members choose a kind of ticket first and which kinds, and which roles are added when a human is needed — one or several, since a server may want its staff and its moderators both; for the help channel, its name and category. Only then does it show the whole plan as sentences and wait for confirmation, through the same door as Command mode, so the same log records who agreed. Nothing is created without an explicit yes on a plan the owner has seen. Changing the choice later never deletes: what the previous choice created is archived, and the owner is told what was archived. The ticket flow — a button opens a private room `#ticket-N` for the member, the human role and Kalvard; a Close button archives it — runs only when `support_mode` is `tickets`. Units cover every mode's questions and plans, and the switch from tickets to an existing channel archiving the button channel; `eval:command` runs the ticket plan's three new actions through the runner.
13. Marketing: inbox rows, bot card, actions, audiences, pricing, footer.
14. Mobile pass at 375px, then a final pass removing one thing from every section. On the hero and onboarding: cap the starfield at 5000 points, no bloom, and fall back to a single rendered still when the first frames measure under 40fps, the fallback being visually identical at rest. Onboarding at 375: the scene takes the top with the beacon smaller, the glass panel docked full width at the bottom; when the keyboard opens the scene shrinks and the panel rises so the current question and the input stay visible, measured with `visualViewport` rather than vh units, and nothing scrolls behind the keyboard.
