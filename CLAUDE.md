...
6. **The beacon, everywhere it belongs.** The object from moment 2 is the product's one device, and it is the same component, the same geometry, the same materials and the same post pass wherever it appears; nothing new is drawn for it. Its light has four states: **off**, unconfigured; **amber**, the vard watching; **working**, amber with a slow pulse (1.4s, shallow) for the seconds while something is actually being carried out; and **green**, the result of an answer. Green is never a decoration. The slit lights in fifths from the bottom, so setup can show what has been decided rather than describe it.
   ...
   **Setup is its own place**, at `/setup/[guildId]`: no sidebar, no dashboard chrome, one decision on screen at a time, and the hero's scene held for the whole flow — the cliff, the coded sky, the beacon standing right of centre on the ground with its contact shadow, large (about 55% of the viewport's height), and never moving between steps. There is no paper and no flat page anywhere in it. The slit is the progress: dark at the door, one fifth in amber per thing decided (name, voice, language, knowledge, scope), from the bottom up; full amber at the test; a crossfade to green when the bot arrives in the server. No other progress bar and no "n of 5" line: the object says it. The door is one line, "Kalvard isn't set up yet.", and one button, centred in the sky; the form is offered once, quietly, inside the first question, and nowhere else. Every step lives on smoked glass on the left half of the scene (night at 60%, blur 6px, one 1px specular top edge, radius 16, the night shadow; max 620 wide, vertically centred, 32px padding): Kalvard's questions at 22px in star white with the green rule, the owner's answers as smaller right-aligned bubbles on lighter glass, the input inside the panel at its bottom, 52px, transparent, a hairline that turns amber on focus, Send inside it on the right. What has been decided stands on a second glass right of the beacon, listing only the fields that exist. Setup's buttons are 52px tall, 24px sides, 16px text, radius 12, star white with ink text and a 1px specular top edge, hover brightens 6%, active scales 0.98, centred under the text on the door and the success screen. On a phone the glass is full width along the bottom with the beacon behind it. Reduced motion shows every end state and animates nothing. `/dev/setup?step=` shows any step without a session.
