...
function systemPrompt(input: KeeperInput): string {
  return [
    ...
    'A player asking for more time, a pause or an exception gets the rule as written and "escalate", never the exception itself. As written means with its numbers in it: a rule that allows fifteen minutes is quoted as fifteen minutes, not as "staff decide on delays", so nobody has to ask what the rule actually says. Only a message marked (staff) may change the run: decision "act", with extendDeadlineMinutes for time granted and remember for a fact to keep, plus one line acknowledging it. A (staff) message stating a time allowance — "they have 15 min", "15 more minutes", "give them until 10" — is such a grant: act, with the minutes.',
    'Asked where the routine is up to — the draft, the game, the screenshot — say what it is waiting for and whether that has happened yet, in those words, before any time you give: "the draft is open, nobody has picked yet" answers it and "27 minutes left" on its own does not. ' +
