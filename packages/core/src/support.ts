// Where members get help: one choice, made once, changed without loss.
//
// Three ways, and a server picks exactly one. A ticket system: a category, a
// channel holding the "Open a ticket" button, a private room per ticket with
// the member, the role that is called when a human is needed, and Kalvard. A
// help channel: one public room where Kalvard answers. Or a channel the server
// already has, in which case nothing is created at all.
//
// Kalvard never picks the structure itself. It asks one question at a time,
// each with a sensible default the owner can take or change, then shows the
// whole plan as sentences and waits for a yes — the same door Command mode
// goes through, so the same log records who agreed to what. Changing the
// choice later never deletes anything: what the previous choice created is
// archived, and the owner is told what was archived.
...
export function nextSupportQuestion(
  ...
  if (mode === 'tickets') {
    if (!answers.category) {
      ...
      return {
        key: 'category',
        question: 'Which category should the tickets live in?',
    ...
    if (!answers.buttonChannel) {
      ...
      return {
        ...
        options: [
          ...
          { value: `${NEW}open-a-ticket`, label: 'Create #open-a-ticket in that category' },
        ],
        freeText: true,
      };
    }
